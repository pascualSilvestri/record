...
import 'package:record/widgets/boxPlay.dart';
  ...
  void stopRecorder() async {
    await _mRecorder!.stopRecorder().then((value) {
      setState(() {
        ...
        Audio audio = Audio(url: cont.toString()+_mPath);
       final audioListProvider = context.read<AudioListProvider>();
       audioListProvider.addAudio(audio);
      });
    });
  }
...
_Fn? getPlaybackFn(Audio audioToPlay) {
  if (!_mPlayerIsInited || !_mplaybackReady || !_mRecorder!.isStopped) {
    return null;
  }
  return () {
    if (_mPlayer!.isStopped) {
      // Iniciar la reproducción del audio desde el Provider.
      _mPlayer!.startPlayer(
        fromURI: audioToPlay.url, // Utiliza la URL del audio desde el Provider.
        whenFinished: () {
          setState(() {}); // Actualiza el estado después de la reproducción.
        },
      );
    } else {
      ...
      return ListView(
        children: [
          ...
          for (var i = 0; i < audio.length; i++)
            BoxPlay(getPlaybackFn: getPlaybackFn, mPlayer: _mPlayer!, audioToPlay: audio[i]),
          
          Column(
            children: [
              for (var i = 0; i < audio.length; i++)
                ListTile(
                  title: Text(audio[i].url),
                )
            ],
          )
        ],
